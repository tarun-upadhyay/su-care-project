...
const SignInFormComponent = () => {
  ...
  async function handleSubmit(values: any) {
    ...
    try {
      await axios
        .post(`/backend/api/v1/auth/login`, values)
        .then((res) => {
          signIn("credentials", {
            email: values.email,
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="mt-8">
      ...
      <div className="space-y-5 font-baijam font-bold">
        <div>
          <label htmlFor="" className="text-base word-spacing-2 text-gray-900">
            {" "}
            Email address{" "}
          </label>
          <div className="mt-2 ">
        ...
        <div>
          <button
            ...
            className={`inline-flex w-full items-center justify-center rounded-md  px-3.5 py-2.5 font-semibold leading-7 text-white hover:bg-black/80  ${
              loading ? "bg-gray-600 cursor-not-allowed opacity-50" : "bg-black"
            }`}
            disabled={loading}
          >
